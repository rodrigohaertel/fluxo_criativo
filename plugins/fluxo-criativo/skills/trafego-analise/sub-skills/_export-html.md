...
Onde `{RAIZ_PROJETO}` é o caminho absoluto da raiz do workspace (descoberta no início da sessão e usada sempre — nunca relativo, nunca `${CLAUDE_PLUGIN_ROOT}/scripts/...`).
...
| ❌ Errado | ✅ Certo | Motivo |
|---|---|---|
| `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/abrir-html.py "..."` | `python3 "{RAIZ}/scripts/abrir-html.py" "..."` | cwd pode ter sido alterado por `cd` anterior |
...
| `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/abrir-html.py` | absoluto sempre | `./` depende de cwd |

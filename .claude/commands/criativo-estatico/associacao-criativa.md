...
#### C) Prompt pro Freepik (Magnific) (animação image-to-video)
...
Depois de entregar o Prompt Feed e o Prompt Freepik (Magnific), pergunte:
...
[prompt Freepik (Magnific), dentro de bloco de código]
...
4. Salve a imagem gerada (Feed ou Stories) e suba no Freepik (Magnific) (ferramenta image-to-video). Cole o **Prompt Freepik (Magnific)** como prompt de animação. O Freepik (Magnific) vai entregar um vídeo curto em loop com a arte animada de forma sutil, sem cortar nem mexer no texto.
...
a) Grave o prompt de animação num arquivo `.txt` na pasta de criativos, com o nome `prompt-associacao-criativa-{numero}-loop.txt`. Use o Prompt de Animação do formato; se não houver, use o prompt padrão abaixo.
...
```bash
py -3 scripts/animar-criativo.py --image "meus-produtos/{ativo}/entregas/criativos/criativo-associacao-criativa-{numero}-feed.png" --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-associacao-criativa-{numero}-loop.txt" --out "meus-produtos/{ativo}/entregas/criativos/criativo-associacao-criativa-{numero}-loop.mp4"
```
...
Vídeo: {caminho-raiz-projeto}\meus-produtos\{ativo}\entregas\criativos\criativo-associacao-criativa-{numero}-loop.mp4
```

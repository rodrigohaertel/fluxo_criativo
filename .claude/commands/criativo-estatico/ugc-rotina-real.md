...
2 a 3 linhas em primeira pessoa, tom de creator amador brasileira. Gera curiosidade sem entregar tudo. Termina com "Clica em Saiba mais que te conto tudo 👇" ou similar. Light Copy aplicada.
...
NA BASE DA FOTO, um CTA simples e discreto estilo creator amador (sem botão, sem caixa, só texto com emoji): "👇 te conto tudo, clica em Saiba mais"
...
REGRA CRÍTICA: as duas caixinhas pretas de legenda no estilo TikTok (com headline e resposta) e o CTA "👇 te conto tudo, clica em Saiba mais" são ESTÁTICAS. Não balançam, não aparecem com animação, não se movem. Ficam fixas o tempo todo. Só a pessoa e o cenário por trás é que têm o micro-movimento natural.
...
```bash
py -3 scripts/gerar-criativo-estatico.py --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-ugc-rotina-real-{numero}-feed.txt" --model "{modelo}" --aspect "4:5" --out "meus-produtos/{ativo}/entregas/criativos/criativo-ugc-rotina-real-{numero}-feed.png"
```
...
a) Grave o prompt de animação num arquivo `.txt` na pasta de criativos, com o nome `prompt-ugc-rotina-real-{numero}-loop.txt`. Use o Prompt de Animação do formato; se não houver, use o prompt padrão abaixo.
...
```bash
py -3 scripts/animar-criativo.py --image "meus-produtos/{ativo}/entregas/criativos/criativo-ugc-rotina-real-{numero}-feed.png" --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-ugc-rotina-real-{numero}-loop.txt" --out "meus-produtos/{ativo}/entregas/criativos/criativo-ugc-rotina-real-{numero}-loop.mp4"
```
...
Vídeo: {caminho-raiz-projeto}\meus-produtos\{ativo}\entregas\criativos\criativo-ugc-rotina-real-{numero}-loop.mp4
```
...
- **Exceção documentada — emoji no CTA da legenda:** o emoji 👇 no CTA da legenda ("Clica em Saiba mais que te conto tudo 👇" ou similar) é permitido porque integra o padrão visual de creator amador brasileiro e faz parte da autenticidade do formato UGC. Essa exceção NÃO vale para o título do anúncio, que segue Light Copy sem emojis.
...
- **CTA com regra de contraste contextual**: a cor do CTA adapta ao fundo da imagem pra garantir legibilidade. Estilo "👇 te conto tudo, clica em Saiba mais", sem botão.

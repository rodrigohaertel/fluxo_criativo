...
2 a 3 linhas em primeira pessoa, tom de creator amador. Gera curiosidade sem entregar tudo. Termina com "Clica em Saiba mais que te conto tudo 👇" ou similar. Light Copy aplicada.
...
NA BASE DA FOTO, um CTA simples e discreto estilo creator amador: "👇 te conto tudo, clica em Saiba mais"
...
```bash
py -3 scripts/gerar-criativo-estatico.py --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-jeito-certo-errado-{numero}-feed.txt" --model "{modelo}" --aspect "4:5" --out "meus-produtos/{ativo}/entregas/criativos/criativo-jeito-certo-errado-{numero}-feed.png"
```
...
a) Grave o prompt de animação num arquivo `.txt` na pasta de criativos, com o nome `prompt-jeito-certo-errado-{numero}-loop.txt`. Use o Prompt de Animação do formato; se não houver, use o prompt padrão abaixo.
...
```bash
py -3 scripts/animar-criativo.py --image "meus-produtos/{ativo}/entregas/criativos/criativo-jeito-certo-errado-{numero}-feed.png" --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-jeito-certo-errado-{numero}-loop.txt" --out "meus-produtos/{ativo}/entregas/criativos/criativo-jeito-certo-errado-{numero}-loop.mp4"
```
...
Vídeo: {caminho-raiz-projeto}\meus-produtos\{ativo}\entregas\criativos\criativo-jeito-certo-errado-{numero}-loop.mp4
```

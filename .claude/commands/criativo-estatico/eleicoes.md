...
2 a 4 linhas. Conecta a metáfora eleitoral ao nicho sem entregar tudo. Termina com a chamada do CTA escolhido, padrão **"Saiba mais"**. Light Copy aplicada (sem travessão, sem exclamação, sem pergunta no início, sem promessa vaga).
...
```bash
py -3 scripts/gerar-criativo-estatico.py --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-eleicoes-{numero}-feed.txt" --model "{modelo}" --aspect "4:5" --out "meus-produtos/{ativo}/entregas/criativos/criativo-eleicoes-{numero}-feed.png"
```
...
a) Grave o prompt de animação num arquivo `.txt` na pasta de criativos, com o nome `prompt-eleicoes-{numero}-loop.txt`. Use o Prompt de Animação do formato; se não houver, use o prompt padrão abaixo.
...
```bash
py -3 scripts/animar-criativo.py --image "meus-produtos/{ativo}/entregas/criativos/criativo-eleicoes-{numero}-feed.png" --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-eleicoes-{numero}-loop.txt" --out "meus-produtos/{ativo}/entregas/criativos/criativo-eleicoes-{numero}-loop.mp4"
```
...
Vídeo: {caminho-raiz-projeto}\meus-produtos\{ativo}\entregas\criativos\criativo-eleicoes-{numero}-loop.mp4
```

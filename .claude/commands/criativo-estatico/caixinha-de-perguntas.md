...
2 a 3 linhas. Gera curiosidade sem entregar a resposta toda. Termina com a chamada do CTA escolhido, padrão **"Saiba mais"**. Light Copy aplicada.
...
a) Grave o prompt de animação num arquivo `.txt` na pasta de criativos, com o nome `prompt-caixinha-{numero}-loop.txt`. Use o Prompt de Animação do formato; se não houver, use o prompt padrão abaixo.
...
```bash
py -3 scripts/animar-criativo.py --image "meus-produtos/{ativo}/entregas/criativos/criativo-caixinha-{numero}-feed.png" --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-caixinha-{numero}-loop.txt" --out "meus-produtos/{ativo}/entregas/criativos/criativo-caixinha-{numero}-loop.mp4"
```
...
Vídeo: {caminho-raiz-projeto}\meus-produtos\{ativo}\entregas\criativos\criativo-caixinha-{numero}-loop.mp4
```

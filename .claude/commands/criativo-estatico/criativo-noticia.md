...
2 a 3 linhas, conecta com a manchete, gera curiosidade sem entregar a resposta. Termina com a chamada do CTA escolhido, padrão "Saiba mais". Light Copy aplicada.
...
````
Anima essa imagem com estética de portal de notícia ao vivo. APENAS detalhes da página (foto principal, ticker) se mexem. A manchete, o nome do portal, a data, o subtítulo e qualquer CTA ficam 100% ESTÁTICOS, não se movem em nenhum momento.

MOVIMENTO DA CENA:
A foto principal da matéria tem micro-movimento natural (pessoa respirando, leve panning de 1-2%, leve zoom contemplativo). Pode aparecer um ticker rolando devagar na base da página com manchetes adicionais (texto deslizando da direita pra esquerda, no padrão Globo News / CNN Brasil), MAS o ticker é separado e secundário. Pode ter logo "AO VIVO" piscando suavemente no canto. Loop de 5-7 segundos, padrão de transmissão jornalística calma.

REGRA CRÍTICA: a manchete principal grande, o nome do portal (fictício, não real), a data, o subtítulo, o CTA e qualquer elemento que carregue a informação principal são ESTÁTICOS. Não balançam, não aparecem com animação, não se movem. Ficam fixos o tempo todo pra preservar a credibilidade visual de notícia. Só a foto principal e o ticker secundário é que têm movimento.
...
MÚSICA DE FUNDO SUGERIDA: trilha de telejornal calmo, instrumental, sem letra. Padrão de "matéria de TV" ou "transmissão jornalística". Beat sóbrio, presença leve. Sem letra que possa concorrer com a manchete.
````
...
a) Grave o prompt de animação num arquivo `.txt` na pasta de criativos, com o nome `prompt-noticia-{numero}-loop.txt`. Use o Prompt de Animação do formato; se não houver, use o prompt padrão abaixo.
...
```bash
py -3 scripts/animar-criativo.py --image "meus-produtos/{ativo}/entregas/criativos/criativo-noticia-{numero}-feed.png" --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-noticia-{numero}-loop.txt" --out "meus-produtos/{ativo}/entregas/criativos/criativo-noticia-{numero}-loop.mp4"
```
...
Vídeo: {caminho-raiz-projeto}\meus-produtos\{ativo}\entregas\criativos\criativo-noticia-{numero}-loop.mp4
```

...
2 a 3 linhas. Reforça o valor das dicas e gera curiosidade sem entregar tudo. Termina com a chamada do CTA escolhido, padrão **"Saiba mais"**. Light Copy aplicada.
...
a) Grave o prompt de animação num arquivo `.txt` na pasta de criativos, com o nome `prompt-centro-das-atencoes-{numero}-loop.txt`. Use o Prompt de Animação do formato; se não houver, use o prompt padrão abaixo.
...
```bash
py -3 scripts/animar-criativo.py --image "meus-produtos/{ativo}/entregas/criativos/criativo-centro-das-atencoes-{numero}-feed.png" --prompt-file "meus-produtos/{ativo}/entregas/criativos/prompt-centro-das-atencoes-{numero}-loop.txt" --out "meus-produtos/{ativo}/entregas/criativos/criativo-centro-das-atencoes-{numero}-loop.mp4"
```
...
Vídeo: {caminho-raiz-projeto}\meus-produtos\{ativo}\entregas\criativos\criativo-centro-das-atencoes-{numero}-loop.mp4
```
